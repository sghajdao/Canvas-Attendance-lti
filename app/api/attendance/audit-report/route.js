...
export async function GET(request) {
  ...
  try {
    ...
    if (course_sis_id && date) {
      // Get audit for specific course and date
      ...
      // Get audit history for specific student
      query = await pool.request()
      ...
      .query(`
        SELECT 
          ...
          change_type
        FROM attendance_audit
        WHERE student_sis_id = @studentSisId
        ORDER BY changed_at DESC
        LIMIT 50
      `);
    } else {
      return NextResponse.json({ error: 'Please provide course_sis_id and date, or student_sis_id' }, { status: 400 });
    }
